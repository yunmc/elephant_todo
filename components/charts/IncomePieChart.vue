<template>
  <div class="chart-card">
    ...
    <ClientOnly>
      ...
      <div v-else class="chart-empty">本月暂无收入记录</div>
    </ClientOnly>
  </div>
</template>
...
const COLORS = ['#36B37E', '#00B8D9', '#6554C0', '#FF8B00', '#0065FF', '#4C9AFF', '#ABF5D1', '#79E2F2']
...
const incomeCategories = computed(() =>
  props.byCategory
    .filter(c => c.type === 'income' && Number(c.total) > 0)
    .sort((a, b) => Number(b.total) - Number(a.total)),
)

const hasData = computed(() => incomeCategories.value.length > 0)

const chartOption = computed<EChartsOption>(() => {
  let data = incomeCategories.value.map((c, i) => ({
  ...
  return {
    ...
    graphic: [{
      ...
      top: '36%',
      style: {
        text: `¥${Number(props.totalIncome).toFixed(0)}`,
        textAlign: 'center',
        fontSize: 18,
        fontWeight: 'bold',
